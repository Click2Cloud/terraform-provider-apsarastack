...
func resourceApsaraStackLogStoreIndexCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExpectedErrors(err, []string{LogClientTimeout}) {
				...
				return resource.RetryableError(WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_store", "GetIndex", ApsaraStackSdkGoERROR))
			}
			if !IsExpectedErrors(err, []string{"IndexConfigNotExist"}) {
				return resource.NonRetryableError(WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_store", "GetIndex", ApsaraStackSdkGoERROR))
			}
		}
		if raw != nil {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_log_store_index", "CreateIndex", ApsaraStackSdkGoERROR)
	}
...
func resourceApsaraStackLogStoreIndexUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if update {
		...
		if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
			...
			return WrapErrorf(err, DefaultErrorMsg, d.Id(), "UpdateIndex", ApsaraStackSdkGoERROR)
		}
	}
...
func resourceApsaraStackLogStoreIndexDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		...
		return WrapErrorf(err, DefaultErrorMsg, d.Id(), "DeleteIndex", ApsaraStackSdkGoERROR)
	}
	return nil
}
